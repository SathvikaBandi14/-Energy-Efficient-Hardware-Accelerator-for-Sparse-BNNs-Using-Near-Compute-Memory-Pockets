...
void compute_conv1(
    ...
    hls::stream<ap_uint<1>> bin_stream("bin1");
    hls::stream<ap_uint<CONV1_COLS>> patch_stream("patch1");
    hls::stream<ap_uint<CONV1_ROWS>> raw_out("raw1");

    #pragma HLS STREAM variable=bin_stream depth=784
    #pragma HLS STREAM variable=patch_stream depth=676
    #pragma HLS STREAM variable=raw_out depth=676

    binarize(in_pixels, bin_stream);
    line_buffer_bin_3x3(bin_stream, patch_stream);

    // THRESH_IN_W (last template arg) should match your conv1_thresholds_int bit-width
    sparse_bnn_layer<
        CONV1_ROWS, CONV1_COLS, CONV1_NNZ,
        676, CONV1_POP_W, CONV1_THRESH_W, 10 
    >(
        patch_stream,
        raw_out,
        conv1_cols,
    ...
    #ifndef __SYNTHESIS__
    std::cout << "\n--- CONV1 Heatmap (Avg Density across 32 Channels) ---" << std::endl;
    for (int r = 0; r < 26; r++) {
        for (int c = 0; c < 26; c++) {
            ap_uint<CONV1_ROWS> val = raw_out.read();
            
            // Calculate Density for this 26x26 pixel location
            int bit_count = 0;
            for(int b = 0; b < CONV1_ROWS; b++) {
                if(val[b] == 1) bit_count++;
            }
            float density = (float)bit_count / CONV1_ROWS;

            // Heatmap visualization
            if (density == 0.0)      std::cout << ". "; // Empty
            else if (density < 0.2)  std::cout << "- "; // Very Sparse
            else if (density < 0.5)  std::cout << "x "; // Moderate
            else if (density < 0.8)  std::cout << "# "; // Dense
            else                     std::cout << "@ "; // Saturated

            raw_out.write(val); // Write back so Maxpool can use it
        }
        std::cout << std::endl;
    }
    #endif
    ...
    // Pool: 26x26 -> 13x13
    binary_maxpool_2x2<CONV1_ROWS, 26>(raw_out, out_bits);
}// ============================================================
// CONV2: 13x13 -> 11x11 -> 5x5
// ============================================================
void compute_conv2(
    hls::stream<ap_uint<CONV2_CH>> &in_pixels,
    hls::stream<ap_uint<CONV2_ROWS>> &out_bits
    ...
    hls::stream<ap_uint<CONV2_COLS>> patch_stream("patch2");
    hls::stream<ap_uint<CONV2_ROWS>> raw_out("raw2");

    #pragma HLS STREAM variable=patch_stream depth=121
    #pragma HLS STREAM variable=raw_out depth=121

    line_buffer_multi_valid<CONV2_CH, 13>(in_pixels, patch_stream);

    // CORRECTED: NUM_PIXELS = 121, removed weights_in
    sparse_bnn_layer<
        CONV2_ROWS, CONV2_COLS, CONV2_NNZ,
        121, CONV2_POP_W, CONV2_THRESH_W, 10
    >(
        patch_stream,
        raw_out,
        conv2_cols,
    ...
    #ifndef __SYNTHESIS__
    std::cout << "\n--- CONV2 Heatmap (Avg Density per Pixel) ---" << std::endl;
    for (int r = 0; r < 11; r++) {
        for (int c = 0; c < 11; c++) {
            ap_uint<CONV2_ROWS> val = raw_out.read();
            
            // Count how many bits are '1' in this 64-bit pixel
            int bit_count = 0;
            for(int b = 0; b < CONV2_ROWS; b++) {
                if(val[b] == 1) bit_count++;
            }
            
            float density = (float)bit_count / CONV2_ROWS;

            // Visual Heatmap: 
            // . = low density, # = high density, @ = saturated
            if (density == 0.0)      std::cout << ". ";
            else if (density < 0.2)  std::cout << "- ";
            else if (density < 0.5)  std::cout << "x ";
            else if (density < 0.8)  std::cout << "# ";
            else                     std::cout << "@ ";

            raw_out.write(val); // Push back for Maxpool
        }
        std::cout << std::endl;
    }
#endif

    binary_maxpool_2x2<CONV2_ROWS, 11>(raw_out, out_bits);
}

// ============================================================
// CONV3: 5x5 -> 3x3 -> 1x1 (Global)
// ============================================================
// ============================================================
// CONV3: Heatmap + Detailed Density
// ============================================================
void compute_conv3(
    ...
    hls::stream<ap_uint<CONV3_COLS>> patch_stream("patch3");
    hls::stream<ap_uint<CONV3_ROWS>> raw_out("raw3");
    hls::stream<ap_uint<CONV3_ROWS>> pool_stream("pool3");

    #pragma HLS STREAM variable=patch_stream depth=9
    #pragma HLS STREAM variable=raw_out depth=9
    #pragma HLS STREAM variable=pool_stream depth=9

    line_buffer_multi_valid<CONV3_CH, 5>(in_pixels, patch_stream);

    sparse_bnn_layer<
        CONV3_ROWS, CONV3_COLS, CONV3_NNZ,
        9, CONV3_POP_W, CONV3_THRESH_W, 10
    >(
        patch_stream, raw_out, conv3_cols, conv3_row_ptr, conv3_thresholds_int
    );

#ifndef __SYNTHESIS__
    std::cout << "\n--- CONV3 Heatmap (Avg Density) ---" << std::endl;
#endif

    // --- DEBUG + FORWARD TO POOL ---
    for (int r = 0; r < 3; r++) {
        for (int c = 0; c < 3; c++) {
            ap_uint<CONV3_ROWS> val = raw_out.read();

            // Forward to pooling stream
            pool_stream.write(val);
            ...
            float density = (float)bit_count / CONV3_ROWS;
    ...
    // --- GLOBAL K-OF-N POOLING (3x3, K=3) ---
    global_maxpool<CONV3_ROWS, 3, 2>(
        pool_stream,
        out_bits
    );
}
// ============================================================
// FC1: 128 inputs -> 256 outputs
// ============================================================
void compute_fc1(
    ...
    #ifndef __SYNTHESIS__
    std::cout << "--- Starting FC1 Computation ---" << std::endl;
    #endif

    sparse_bnn_layer<
        FC1_ROWS, FC1_COLS, FC1_NNZ,
        1, FC1_POP_W, FC1_THRESH_W, 10
    >(
        in_vector,
        out_bits,
        fc1_cols,
...
// ============================================================
void compute_fc2(
    hls::stream<ap_uint<256>> &in_vector, 
    hls::stream<ap_uint<10>> &out_digit
) {
    ap_uint<256> input_bits = in_vector.read();
    int max_score = -32768; 
    int winner = 0;

    #ifndef __SYNTHESIS__
    std::cout << "\n--- FC2 Classification Scores ---" << std::endl;
    #endif

    for (int r = 0; r < 10; r++) {
        #pragma HLS PIPELINE II=1
        
        unsigned int start = fc2_row_ptr[r];
        unsigned int end = fc2_row_ptr[r+1];
        
        int matches = 0;
        for (unsigned int i = start; i < end; i++) {
            #pragma HLS UNROLL factor=16
            if (input_bits[fc2_cols[i]] == 1) {
                matches++;
            }
        }

        // CORRECTED: Score = Matches - Folded Threshold
        int score = matches - (int)fc2_thresholds_int[r];

        #ifndef __SYNTHESIS__
        std::cout << "Digit [" << r << "]: Score = " << std::setw(5) << score 
                  << " (Matches: " << matches << " / " << (end-start) << ")" << std::endl;
        #endif
